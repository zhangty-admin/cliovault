...
- 自动、完整保存近期复制过的文本、图片和文件；大段 SQL、JSON、代码和日志不会被截断。
- 快速检索并复用历史内容。
- 通过置顶和分组保留重要内容。
- 数据仅保存在本机用户目录。
...
- 进入具体分组后，可以通过卡片右上角的拖动手柄调整该分组内的数据顺序。
- 排序会复用并持久化现有历史顺序；非当前分组的数据保持原有相对位置。
...
500 条是活动历史面板的容量规则，不是单条内容大小限制。文本达到 256 KiB 后会改用独立内容文件持久化，但在内存模型、搜索、复制、粘贴和编辑时仍是完整文本。
...
1. 立即隐藏面板。
2. 等待约 200ms，让此前的目标窗口真正恢复焦点。
3. 将选中记录写入系统剪贴板。
4. 如果目标窗口标题包含 `JumpServer`、记录为文本且不超过 100 个字符，直接发送 Unicode 键盘文本，绕过异步远程剪贴板桥接。
5. 超过 100 个字符的 JumpServer 文本以及其他目标，等待约 300ms 后按原逻辑模拟 `Ctrl + V` 粘贴。
...
- 删除分组。
- 拖拽调整标签显示顺序。
- 进入具体分组后，通过卡片右上角的 `⠿` 手柄拖动调整分组内记录顺序。
...
- 分组内排序会调整记录在历史集合中的相对位置，因此多标签记录的新位置也会反映到它所属的其他分组和“全部”历史中。
- 只有选中具体分组且搜索框为空时才显示拖动手柄，避免对搜索结果子集进行不完整排序。
- 置顶和非置顶记录不能跨区域拖动，继续保证置顶记录始终位于普通记录之前。
...
- 导出生成 `.zip`，包含 `history.json`、`settings.json`、最近 5 个历史快照、`images/` 和 `contents/`。
- 导入前会预览备份包是否包含历史、设置、图片数量和独立大文本数量。
...
- `history.json`：记录元数据、普通文本、文件路径、置顶状态、多标签、智能类型、回收站及标签和历史记录顺序。达到 256 KiB 的文本只保存 `ContentPath` 和 UTF-8 字节数，不重复嵌入全文。
- `contents`：完整保存达到 256 KiB 的 SQL、JSON、代码、日志等大文本。编辑大文本时创建新的内容版本，避免覆盖历史快照仍引用的旧版本。
- `images`：所有带图片预览的记录对应的 PNG，包括 Image、Rtf、Html 和单图片文件预览。

### 9.2 保存时机

- 历史或标签变化后采用 2 秒防抖，再创建当时的数据快照交给后台单写入队列，不占用界面线程执行 JSON 序列化和磁盘写入。
- 后台正在保存时出现的新变化会替换尚未开始的旧请求；当前写入完成后保存最新快照，不并发写同一份历史文件。
- 应用正常退出时停止防抖、提交最新快照，并等待后台保存完成后退出。
- 设置修改后立即保存。
- JSON 直接流式写入临时文件并刷新到磁盘，不在内存中生成完整 JSON 字符串；完成后再替换正式文件。
- 每次替换前轮转最近 5 个历史快照：`history.json.bak1` 至 `history.json.bak5`。保存过程不再为了验证旧文件而将其全文读取并反序列化。
- 图片和大文本使用独立文件；同一条未编辑的大文本不会在每次保存时重复写入。

### 9.3 加载与降级

- 启动时先恢复置顶记录，再恢复普通记录。
- 主历史文件缺失或损坏时，依次尝试从 `bak1` 到 `bak5` 恢复。
- 版本 3 及更早的内嵌大文本仍可直接加载；下一次保存时自动迁移为版本 4 的 `contents` 独立文件，文本内容、标签、置顶和回收站状态保持不变。
- 版本 4 记录存在 `ContentPath` 时，从 `contents` 加载完整文本，因此现有搜索、复制、粘贴和编辑逻辑不需要区分普通文本与大文本。
...
持久化 DTO 在上述字段之外增加 `ContentPath` 和 `TextLengthBytes`。它们只负责定位独立大文本文件和记录字节数，不进入运行时 `ClipboardItem`；因此上层业务始终通过 `Text` 使用完整内容。
...
12. 大文本虽然从 JSON 拆分到 `contents`，启动时仍会加载完整文本到内存，以保持全文搜索和复制行为；当前尚未实现归档分页或按需加载。
...
| `PersistenceService`      | 后台保存队列、历史 JSON、独立大文本和图片读写 |
| `ImportExportService`     | 包含独立大文本的本地 zip 导入导出和导入前自动备份 |
